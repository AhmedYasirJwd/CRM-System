...
const ClientList = ({ profile }) => {
  const [clients, setClients] = useState({ inConvo: [], declined: [], notReplied: [] });

  const fetchClients = async () => {
    try {
      const querySnapshot = await getDocs(collection(db, `clients_${profile}`));
      const inConvo = [], declined = [], notReplied = [];

      querySnapshot.forEach((docSnap) => {
        const data = docSnap.data();
        if (data.status === "in-convo") inConvo.push({ id: docSnap.id, ...data });
        else if (data.status === "declined") declined.push({ id: docSnap.id, ...data });
  ...
  const renderTable = (title, data, color, actions = false) => (
    <div className="mb-8">
      ...
      {data.length === 0 ? <p className="text-gray-400">None</p> : (
        <div className="overflow-x-auto rounded-lg shadow border border-indigo-400">
          <table className="min-w-full text-gray-100">
            ...
            <tbody className="bg-gray-900 divide-y divide-gray-700">
              {data.map((client) => (
                <tr key={client.id} className="hover:bg-gray-800 transition duration-200">
                  <td className="px-4 py-2 font-medium">{client.name}</td>
                  <td className="px-4 py-2">{client.location}</td>
                  <td className="px-4 py-2">{client.timeZone || "N/A"}</td>
                  <td className="px-4 py-2">{client.whatsappNo || "None"}</td>
                  <td className="px-4 py-2">{client.facebookUrl && client.facebookUrl !== "none" ? <a href={client.facebookUrl} target="_blank" rel="noreferrer" className="text-indigo-400 hover:underline">{client.facebookUrl}</a> : "None"}</td>
                  <td className="px-4 py-2">{client.source}</td>
                  <td className="px-4 py-2">{client.followUps}</td>
                  {actions && (
                    <td className="px-4 py-2 space-x-2">
                      <button onClick={() => moveClient(client.id, "in-convo")} className="px-3 py-1 bg-indigo-600 hover:bg-indigo-800 text-white rounded">Move to In-Convo</button>
                      <button onClick={() => moveClient(client.id, "declined")} className="px-3 py-1 bg-red-600 hover:bg-red-800 text-white rounded">Move to Declined</button>
                    </td>
                  )}
                </tr>
              ))}
